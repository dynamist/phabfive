# -*- coding: utf-8 -*-
"""Display functions for apps whose records are plain nested dicts.

Mirrors ``phabfive.display``, with one difference: a repository or project
record is a plain nested dict of scalars, lists and dicts, so every format
here walks that same dict generically instead of naming the sections one by
one. That is what makes the formats agree by construction - rich cannot grow
a field that yaml lacks, because neither knows what the fields are.
...
Repositories, URIs and projects all come through here. Nothing below knows
which it is holding - a record is a dict, a record with a ``_url`` leads with
a Link and one without leads with its first field - so every command that
prints them shares one set of renderers and one ``--format`` switch. Diffusion
was the first app to use it and projects the second, which is when it moved
out of ``phabfive.diffusion``.

``table`` is the one format they do not all share, and not because it knows
anything about the records: a grid needs a list, so only the list commands
register it and a ``show`` falls back to rich. The renderer itself is
...
def display_records(records, output_format, phabfive_instance, tabular=False):
    ...
    Every diffusion and project command that prints records comes through
    here, so a format added or fixed for one of them is added or fixed for
    all.
    ...
    tabular : bool, optional
        ...
        ``repo show`` and ``project show`` set it False and
        `--format=table` falls back to rich there, which is
    ...
    if tabular:
        ...
        # publish, so it has no idea whether it is holding repositories, URIs
        # or projects - and no app needs a column list of its own.
